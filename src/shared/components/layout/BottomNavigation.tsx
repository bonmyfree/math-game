...
/**
 * App navigation.
 * - Large screens (md and up: desktop, tablet, unfolded foldables): fixed left
 *   sidebar with logo header and a collapse/expand toggle.
 * - Phone (below md): fixed bottom navigation bar (always expanded, no collapse).
 *
 * The collapse state only affects the large-screen sidebar.
 */
export function BottomNavigation() {
  ...
  const visibleItems = NAV_ITEMS.filter((item) => hasRightAccess(item.right))

  return (
    <>
      {/* Large screens — left sidebar (collapsible) */}
      <aside
        className={cn(
          'hidden md:flex fixed left-0 top-0 h-full bg-slate-900 text-white flex-col z-30 transition-all duration-300 ease-in-out',
          collapsed ? 'w-16' : 'w-60',
        )}
      >
      ...
      <nav
        className="md:hidden fixed bottom-0 inset-x-0 z-30 flex items-stretch justify-around bg-slate-900 text-white border-t border-slate-700"
        style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}
      >
        {visibleItems.map((item) => {
          const path = item.path ?? item.children?.[0]?.path
          if (!path) return null
          const active = item.children ? isParentActive(item.children) : isActive(path)
          return (
            <Link
              key={item.key}
              to={path}
            ...
            >
              <span className="shrink-0">{getImageByKey(item.icon, 22)}</span>
