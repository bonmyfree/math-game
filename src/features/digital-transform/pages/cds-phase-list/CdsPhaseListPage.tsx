...
import type { CdsPhaseData, CdsPhaseListPayload } from '@/features/digital-transform/services'
...
import { DEFAULT_CDS_PHASE_PAYLOAD } from './constants'
import { phaseCode, phaseDescription, phaseName, phaseProject, rowId } from './rowAccessors'
import { CDS_RIGHT } from '../../routes/constant'

/** Chuy?n d?i s? > Qu?n l? d? ?n C?S > Danh s?ch giai do?n */
export default function CdsPhaseListPage() {
  const { canView, canUpdate } = usePermission(CDS_RIGHT.PHASE_LIST)
  const { t } = useTranslation()

  const [submittedPayload, setSubmittedPayload] =
    useState<CdsPhaseListPayload>(DEFAULT_CDS_PHASE_PAYLOAD)
  const [currentPage, setCurrentPage] = useState(DEFAULT_CDS_PHASE_PAYLOAD.PAGE)
  const [pageSize, setPageSize] = useState(DEFAULT_CDS_PHASE_PAYLOAD.RECORD_PER_PAGE)
  ...
  const {
    ...
    queryKey: ['cdsPhases', submittedPayload],
    queryFn: () => cdsService.getAllPhases(submittedPayload),
    enabled: canView,
  ...
  const handleAdd = useCallback(() => {
    if (!canUpdate) return
    toastService.warning(t('cdsPhase.createNotImplemented'))
  }, [canUpdate, t])

  const handleView = useCallback(
    (row: CdsPhaseData) => {
      toastService.warning(t('cdsPhase.viewNotImplemented', { name: phaseName(row) }))
    },
    [t],
  )
  ...
  const handleReset = useCallback(() => {
    ...
    setSubmittedPayload({ ...DEFAULT_CDS_PHASE_PAYLOAD })
  }, [selection])
  ...
  const columnsDef = useMemo<ColumnDef<CdsPhaseData>[]>(
    () => [
      createSelectColumn<CdsPhaseData>({
        idPrefix: 'cds-phase',
        getRowId: rowId,
      ...
      {
        id: 'phaseName',
        header: t('table.phaseName'),
        size: 200,
        cell: rowTextCell(phaseName, 'line-clamp-2'),
      },
      {
        id: 'project',
        header: t('table.project'),
        size: 160,
        cell: rowTextCell(phaseProject),
      },
      {
        id: 'phaseCode',
        header: t('table.phaseCode'),
        size: 120,
        cell: rowTextCell(phaseCode),
      },
      {
        accessorKey: 'C_START_DATE',
        header: t('table.startDate'),
        size: 120,
        cell: dateCell<CdsPhaseData>(),
      },
      {
      ...
      },
      {
        ...
        size: 220,
        cell: rowTextCell(phaseDescription, 'line-clamp-2'),
      },
      {
  ...
  return (
    <PageGuard functionCode={CDS_RIGHT.PHASE_LIST}>
      <div className="m-4">
        <PageToolbar>
